...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        users_without_email = User.objects.filter(email="") | User.objects.filter(
            email__isnull=True
        )

        if not users_without_email.exists():
            self.stdout.write(
                self.style.SUCCESS("✅ All users already have email addresses!")
            )
            return
        ...
        for user in users_without_email:
            ...
            if email:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  ✅ Updated {user.username} with email: {email}\n"
                    )
                )
            else:
